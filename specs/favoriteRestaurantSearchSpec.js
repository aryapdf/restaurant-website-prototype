...
describe('Searching for Restaurants', () => {
  ...
  describe('When query is not empty', () => {
    ...
    it('should show the found restaurants', () => {
      ...
      presenter._showFoundRestaurants([{ id: 1, name: 'One' }, { id: 2, name: 'Two' }]);
      expect(document.querySelectorAll('.restaurant-item').length)
        .toEqual(2);
    });

    it('should show the name of the found restaurants', () => {
      presenter._showFoundRestaurants([{ id: 1, name: 'One' }]);
      expect(document.querySelectorAll('.restaurant_title')
        .item(0).textContent)
        .toEqual('One');
    });
    ...
    it('should show - when the restaurant returned does not contain a name', (done) => {
      document.getElementById('restaurants').addEventListener('restaurants:updated', () => {
        const restaurantNames = document.querySelectorAll('.restaurant_title');
